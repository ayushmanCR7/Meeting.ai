...
import { CallSessionStartedEvent, CallSessionParticipantLeftEvent, CallEndedEvent, CallTranscriptionReadyEvent, CallRecording, CallRecordingReadyEvent } from "@stream-io/node-sdk";
...
export async function POST(req: NextRequest) {
    ...
    if (eventType === "call.session_started") {
        ...
        if (!existingAgent) {
            return NextResponse.json({ error: "Agnet not found" }, { status: 404 })
        }

        const call = streamVideo.video.call("default", meetingId)

        const realTimeClient = await streamVideo.video.connectOpenAi({
            call, openAiApiKey: process.env.OPEN_AI_API_KEY!, agentUserId: existingAgent.id
        })
